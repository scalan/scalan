...
//  //  }
//
//  val HasViewArg = HasArg(_ match { case Def(ExpViewArray(_,_)) => true case _ => false })
//
//  object View {
...
//  }
//
//  def mapUnderlyingArray[A,B,C](view: ExpViewArray[A,B], f: Rep[B=>C]): PA[C] = {
//    implicit val eB = view.iso.eB
//    implicit val eC: Elem[C] = f.Elem.eb
//    view.arrOrEmpty map { (x: Exp[A]) => f(view.iso.toStaged(x)) }
//  }
...
//    case MapPA(Def(view@ExpViewArray(_, _)), f) => {
...
//    case MethodCall(Def(obj@UserType(_)), m, args) => {
...
//    }
//
//    case MapPA(xs, f@Def(Lambda(_,_,UserTypeSym(iso)))) => {
//      implicit val eC = f.Elem.eb
//      val s = xs map (x => {
//        val tmp = f(x)
//        iso.asInstanceOf[Iso[Any, Any]].fromStaged(tmp)
//      })
//      val res = mkView(s)(iso.asInstanceOf[Iso[Any, Any]])
//      res
//    }
//
//    case MapPA(xs, f@Def(lam@Lambda(_,_,Def(view@ExpViewArray(_, _)))) ) => {
//      implicit val eA = view.iso.eA
//      val s = xs map (x => {
//        unmkView(f(x).asRep[PArray[Any]])(view.iso)
//      })
//      val res = (mkView(s.values)(view.iso)).nestBy(s.segments)
//      res
//    }
//
//    case LoopUntil(start@TupleTree(tree), step, isMatch) if tree.hasViews => {
//      val startTree = tree.eliminateViews
//      implicit val stateElem = startTree.root.asRep[Any].Elem
//      val loopRes: Rep[Any] = LoopUntil(
//        startTree.root,
//        fun { (x: Rep[Any]) => {
//          val x_viewed = tree.toView(x)
//          val res_viewed = step.asFunc[Any,Any,Rep[Any]] { eA => eB => step =>
//            mirrorApply(step, x_viewed)(eA,eB)
//          }
...
//        fun { (x: Rep[Any]) => {
//          val x_viewed = tree.toView(x)
//          val res = isMatch.asFunc[Any,Boolean,Rep[Boolean]] { eA => eB => isMatch =>
//            mirrorApply(isMatch, x_viewed)(eA,eB)
//          }
//          res
