...
object ScalanLiteBuild extends Build {
  ...
  // TODO there is some problem with filtering
  ...
  def perfFilter(name: String) = name.startsWith("tests.performance")

  def itFilter(name: String): Boolean = false // name.contains("ItTests")

  lazy val core = Project("scalan-lite", file("core")).configs(ItTest, PerfTest).settings(commonSettings: _*)
