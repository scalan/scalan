...
//      companion((rows zip other.rows).map { case Pair(v1, v2) => v1 *^ v2 })
  ...
  abstract class DenseFlatMatrix[T]
  (val rmValues: Rep[Collection[T]],
   val numColumns: Rep[Int])(implicit val eT: Elem[T]) extends Matrix[T] {
    def items = rmValues
  ...
  abstract class SparseMatrix[T](
        override val sparseRows: Rep[NestedCollection[(Int,T)]],
        val numColumns: Rep[Int])(implicit val eT: Elem[T]) extends Matrix[T]
  {
    def companion = SparseMatrix
    def rows = sparseRows.map(row => SparseVector1(row, numColumns))
    def columns: Rep[Collection[Vector[T]]] = {
      Collection(SArray.tabulate(numColumns) { j => DenseVector(rows.map(_(j)))})
    }
    def numRows = rows.length
    ...
    @OverloadId("rows")
    def apply(iRows: Coll[Int])(implicit o: Overloaded1): Matr[T] = {
      companion(sparseRows(iRows), numColumns)
    }
    @OverloadId("row")
