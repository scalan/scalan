...
import scalan.meta.{ScalanParsers, EntityManagement, UnitConfig, SourceModuleConf}
...
trait ScalanizerBase[+G <: Global] extends ScalanParsers[G] {
  ...
  def saveCode(module: SourceModuleConf, packageName: String, fileName: String, code: String) = {
    ...
    val resourceFile = FileUtil.file(module.getResourceHome, packagePath, fileName + ".scala")
    resourceFile.mkdirs()
