...
trait Slicing extends Scalan {
  ...
  trait SliceMarking[T] extends Marking[T] { _: Product =>
    ...
    def projectToExp(x: Exp[T]): Sym
    def projectedElem: Elem[_]
    def makeSlot: Exp[T]
    def set(slot: Exp[T], value: Sym): Exp[T]
  ...
  case class EmptyBaseMarking[T](override val elem: Elem[T])
    ...
    def projectToExp(x: Exp[T]): Sym = toRep(())
  ...
  class StructMarking[T <: Struct] private (val fields: Seq[(String, SliceMarking[_])], val elem: Elem[T]) extends SliceMarking[T] with Product {
    ...
    def set(slot: Exp[T], value: Sym) = slot match {
  ...
  case class PairMarking[A,B](markA: SliceMarking[A], markB: SliceMarking[B]) extends SliceMarking[(A,B)] {
    ...
    def projectToExp(x: Exp[(A,B)]): Sym =
  ...
  case class FuncMarking[A,B](mDom: SliceMarking[A], mRange: SliceMarking[B]) extends SliceMarking[A => B] {
    ...
    def projectToExp(f: Exp[A => B]): Sym =
      sliceFunc(f, this)
  ...
  case class TraversableMarking[T, F[_]](itemsPath: KeyPath, innerMark: SliceMarking[T], override val cF: Cont[F]) extends SliceMarking1[T, F]()(cF) {
    ...
    def projectToExp(xs: Exp[F[T]]): Sym = itemsPath match {
      case KeyPath.All =>
  ...
  case class ThunkMarking[A](innerMark: SliceMarking[A]) extends SliceMarking1[A, Thunk] {
    ...
    def projectToExp(thunk: Exp[Thunk[A]]): Sym = thunk match {
...
//    override def mirrorNode[A](t: MapTransformer, rewriter: Rewriter, g: AstGraph, node: Exp[A]): (MapTransformer, Sym) = {
...
//    }
    override protected def mirrorLambda[A, B](
        t: MapTransformer, rewriter: Rewriter, node: Exp[(A) => B], lam: Lambda[A, B]): (MapTransformer, Sym) = {
  ...
  object IsSlicedFunc {
    def unapply[T](s: Exp[T]): Option[(SlicedFunc[_,_,_,_], Sym, SliceMarking[T])] = s match {
  ...
  object Sliced {
    def apply[A](s: Sym, m: SliceMarking[A]): Exp[A] =
