...
!!!("Cannot convert from Converter to SumConverter: missing fields List(conv1, conv2)")
  ...
  // elem for concrete class
  class FunctorConverterElem[A, B, F[_]](val iso: Iso[FunctorConverterData[A, B, F], FunctorConverter[A, B, F]])(implicit eA: Elem[A], eB: Elem[B], cF: Cont[F], F: Functor[F])
...
!!!("Cannot convert from Converter to FunctorConverter: missing fields List(itemConv)")
  ...
  // state representation type
  type FunctorConverterData[A, B, F[_]] = A => B

  // 3) Iso for concrete class
  class FunctorConverterIso[A, B, F[_]](implicit eA: Elem[A], eB: Elem[B], cF: Cont[F], F: Functor[F])
    extends Iso[FunctorConverterData[A, B, F], FunctorConverter[A, B, F]] {
    override def from(p: Rep[FunctorConverter[A, B, F]]) =
      p.itemConv
    override def to(p: Rep[A => B]) = {
      val itemConv = p
      FunctorConverter(itemConv)
    }
    lazy val defaultRepTo: Rep[FunctorConverter[A, B, F]] = FunctorConverter(fun { (x: Rep[A]) => element[B].defaultRepValue })
  ...
  abstract class FunctorConverterCompanionAbs extends CompanionBase[FunctorConverterCompanionAbs] with FunctorConverterCompanion {
    ...
    def apply[A, B, F[_]](itemConv: Rep[A => B])(implicit eA: Elem[A], eB: Elem[B], cF: Cont[F], F: Functor[F]): Rep[FunctorConverter[A, B, F]] =
  ...
  implicit class ExtendedFunctorConverter[A, B, F[_]](p: Rep[FunctorConverter[A, B, F]])(implicit eA: Elem[A], eB: Elem[B], cF: Cont[F], F: Functor[F]) {
    def toData: Rep[FunctorConverterData[A, B, F]] = isoFunctorConverter(eA, eB, cF, F).from(p)
  }

  // 5) implicit resolution of Iso
  implicit def isoFunctorConverter[A, B, F[_]](implicit eA: Elem[A], eB: Elem[B], cF: Cont[F], F: Functor[F]): Iso[FunctorConverterData[A, B, F], FunctorConverter[A, B, F]] =
    new FunctorConverterIso[A, B, F]

  // 6) smart constructor and deconstructor
  def mkFunctorConverter[A, B, F[_]](itemConv: Rep[A => B])(implicit eA: Elem[A], eB: Elem[B], cF: Cont[F], F: Functor[F]): Rep[FunctorConverter[A, B, F]]
  def unmkFunctorConverter[A, B, F[_]](p: Rep[Converter[F[A], F[B]]]): Option[(Rep[A => B])]
}
...
trait ConvertersSeq extends ConvertersDsl  {
  ...
  case class SeqFunctorConverter[A, B, F[_]]
      (override val itemConv: Rep[A => B])
      (implicit eA: Elem[A], eB: Elem[B], cF: Cont[F], F: Functor[F])
  ...
  def mkFunctorConverter[A, B, F[_]]
      (itemConv: Rep[A => B])(implicit eA: Elem[A], eB: Elem[B], cF: Cont[F], F: Functor[F]): Rep[FunctorConverter[A, B, F]] =
