...
trait ViewsAbs extends Views {
  ...
  abstract class AbsReverseIso[A, B]
      (iso: Iso[A, B])(implicit eA: Elem[A], eB: Elem[B])
    extends ReverseIso[A, B](iso) with Def[ReverseIso[A, B]] {
    lazy val selfType = element[ReverseIso[A, B]]
  }
  // elem for concrete class
  class ReverseIsoElem[A, B](val iso: Iso[ReverseIsoData[A, B], ReverseIso[A, B]])(implicit val eA: Elem[A], val eB: Elem[B])
    extends IsoURElem[B, A, ReverseIso[A, B]]
    with ConcreteElem[ReverseIsoData[A, B], ReverseIso[A, B]] {
    override lazy val parent: Option[Elem[_]] = Some(isoURElement(element[B], element[A]))
    override lazy val typeArgs = TypeArgs("A" -> (eA -> scalan.util.Invariant), "B" -> (eB -> scalan.util.Invariant))

    override def convertIsoUR(x: Rep[IsoUR[B, A]]) = // Converter is not generated by meta
!!!("Cannot convert from IsoUR to ReverseIso: missing fields List(iso)")
    override def getDefaultRep = ReverseIso(element[IsoUR[A, B]].defaultRepValue)
    override lazy val tag = {
      implicit val tagA = eA.tag
      implicit val tagB = eB.tag
      weakTypeTag[ReverseIso[A, B]]
    }
  }

  // state representation type
  type ReverseIsoData[A, B] = IsoUR[A, B]

  // 3) Iso for concrete class
  class ReverseIsoIso[A, B](implicit eA: Elem[A], eB: Elem[B])
    extends EntityIso[ReverseIsoData[A, B], ReverseIso[A, B]] with Def[ReverseIsoIso[A, B]] {
    override def from(p: Rep[ReverseIso[A, B]]) =
      p.iso
    ...
    lazy val eTo = new ReverseIsoElem[A, B](self)
    lazy val selfType = new ReverseIsoIsoElem[A, B](eA, eB)
    def productArity = 2
  ...
  case class ReverseIsoIsoElem[A, B](eA: Elem[A], eB: Elem[B]) extends Elem[ReverseIsoIso[A, B]] {
    def getDefaultRep = reifyObject(new ReverseIsoIso[A, B]()(eA, eB))
    lazy val tag = {
      implicit val tagA = eA.tag
      implicit val tagB = eB.tag
      weakTypeTag[ReverseIsoIso[A, B]]
    }
    lazy val typeArgs = TypeArgs("A" -> (eA -> scalan.util.Invariant), "B" -> (eB -> scalan.util.Invariant))
  }
  // 4) constructor and deconstructor
  class ReverseIsoCompanionAbs extends CompanionDef[ReverseIsoCompanionAbs] {
    def selfType = ReverseIsoCompanionElem
    override def toString = "ReverseIso"

    @scalan.OverloadId("fromFields")
    def apply[A, B](iso: Iso[A, B])(implicit eA: Elem[A], eB: Elem[B]): Rep[ReverseIso[A, B]] =
      mkReverseIso(iso)

    def unapply[A, B](p: Rep[IsoUR[B, A]]) = unmkReverseIso(p)
  }
  lazy val ReverseIsoRep: Rep[ReverseIsoCompanionAbs] = new ReverseIsoCompanionAbs
  lazy val ReverseIso: ReverseIsoCompanionAbs = proxyReverseIsoCompanion(ReverseIsoRep)
  implicit def proxyReverseIsoCompanion(p: Rep[ReverseIsoCompanionAbs]): ReverseIsoCompanionAbs = {
    proxyOps[ReverseIsoCompanionAbs](p)
  }

  implicit case object ReverseIsoCompanionElem extends CompanionElem[ReverseIsoCompanionAbs] {
    lazy val tag = weakTypeTag[ReverseIsoCompanionAbs]
    protected def getDefaultRep = ReverseIso
  }

  implicit def proxyReverseIso[A, B](p: Rep[ReverseIso[A, B]]): ReverseIso[A, B] =
    proxyOps[ReverseIso[A, B]](p)

  implicit class ExtendedReverseIso[A, B](p: Rep[ReverseIso[A, B]])(implicit eA: Elem[A], eB: Elem[B]) {
    def toData: Rep[ReverseIsoData[A, B]] = isoReverseIso(eA, eB).from(p)
  }

  // 5) implicit resolution of Iso
  implicit def isoReverseIso[A, B](implicit eA: Elem[A], eB: Elem[B]): Iso[ReverseIsoData[A, B], ReverseIso[A, B]] =
    reifyObject(new ReverseIsoIso[A, B]()(eA, eB))

  // 6) smart constructor and deconstructor
  def mkReverseIso[A, B](iso: Iso[A, B])(implicit eA: Elem[A], eB: Elem[B]): Rep[ReverseIso[A, B]]
  def unmkReverseIso[A, B](p: Rep[IsoUR[B, A]]): Option[(Rep[IsoUR[A, B]])]
...
!!!("Cannot convert from IsoUR to ConverterIso: missing fields List(convTo, convFrom)")
  ...
  class MapIsoElem[K1, V1, K2, V2](val iso: Iso[MapIsoData[K1, V1, K2, V2], MapIso[K1, V1, K2, V2]])(implicit val eK1: Elem[K1], val eV1: Elem[V1], val eK2: Elem[K2], val eV2: Elem[V2])
    ...
    override lazy val typeArgs = TypeArgs("K1" -> (eK1 -> scalan.util.Invariant), "V1" -> (eV1 -> scalan.util.Invariant), "K2" -> (eK2 -> scalan.util.Invariant), "V2" -> (eV2 -> scalan.util.Invariant))
...
!!!("Cannot convert from IsoUR to MapIso: missing fields List(iso1, iso2)")
  ...
  case class MapIsoIsoElem[K1, V1, K2, V2](eK1: Elem[K1], eV1: Elem[V1], eK2: Elem[K2], eV2: Elem[V2]) extends Elem[MapIsoIso[K1, V1, K2, V2]] {
    ...
    lazy val typeArgs = TypeArgs("K1" -> (eK1 -> scalan.util.Invariant), "V1" -> (eV1 -> scalan.util.Invariant), "K2" -> (eK2 -> scalan.util.Invariant), "V2" -> (eV2 -> scalan.util.Invariant))
  }
  // 4) constructor and deconstructor
...
trait ViewsStd extends ViewsDsl {
  ...
  case class StdReverseIso[A, B]
      (override val iso: Iso[A, B])(implicit eA: Elem[A], eB: Elem[B])
    extends AbsReverseIso[A, B](iso) {
  }

  def mkReverseIso[A, B]
    (iso: Iso[A, B])(implicit eA: Elem[A], eB: Elem[B]): Rep[ReverseIso[A, B]] =
    new StdReverseIso[A, B](iso)
  def unmkReverseIso[A, B](p: Rep[IsoUR[B, A]]) = p match {
    case p: ReverseIso[A, B] @unchecked =>
      Some((p.iso))
    case _ => None
  }
...
trait ViewsExp extends ViewsDsl {
  ...
  case class ExpReverseIso[A, B]
      (override val iso: Iso[A, B])(implicit eA: Elem[A], eB: Elem[B])
    extends AbsReverseIso[A, B](iso)

  object ReverseIsoMethods {
    object isIdentity {
      def unapply(d: Def[_]): Option[Rep[ReverseIso[A, B]] forSome {type A; type B}] = d match {
        case MethodCall(receiver, method, _, _) if receiver.elem.isInstanceOf[ReverseIsoElem[_, _]] && method.getName == "isIdentity" =>
          Some(receiver).asInstanceOf[Option[Rep[ReverseIso[A, B]] forSome {type A; type B}]]
        case _ => None
      }
      def unapply(exp: Exp[_]): Option[Rep[ReverseIso[A, B]] forSome {type A; type B}] = exp match {
        case Def(d) => unapply(d)
        case _ => None
      }
    }
    ...
    object from {
      def unapply(d: Def[_]): Option[(Rep[ReverseIso[A, B]], Rep[A]) forSome {type A; type B}] = d match {
        case MethodCall(receiver, method, Seq(a, _*), _) if receiver.elem.isInstanceOf[ReverseIsoElem[_, _]] && method.getName == "from" =>
          Some((receiver, a)).asInstanceOf[Option[(Rep[ReverseIso[A, B]], Rep[A]) forSome {type A; type B}]]
        case _ => None
      }
      def unapply(exp: Exp[_]): Option[(Rep[ReverseIso[A, B]], Rep[A]) forSome {type A; type B}] = exp match {
        case Def(d) => unapply(d)
        case _ => None
      }
    }
    ...
    object to {
      def unapply(d: Def[_]): Option[(Rep[ReverseIso[A, B]], Rep[B]) forSome {type A; type B}] = d match {
        case MethodCall(receiver, method, Seq(b, _*), _) if receiver.elem.isInstanceOf[ReverseIsoElem[_, _]] && method.getName == "to" =>
          Some((receiver, b)).asInstanceOf[Option[(Rep[ReverseIso[A, B]], Rep[B]) forSome {type A; type B}]]
        case _ => None
      }
      def unapply(exp: Exp[_]): Option[(Rep[ReverseIso[A, B]], Rep[B]) forSome {type A; type B}] = exp match {
        case Def(d) => unapply(d)
        case _ => None
      }
    }
  ...
  def mkReverseIso[A, B]
    (iso: Iso[A, B])(implicit eA: Elem[A], eB: Elem[B]): Rep[ReverseIso[A, B]] =
    new ExpReverseIso[A, B](iso)
  def unmkReverseIso[A, B](p: Rep[IsoUR[B, A]]) = p.elem.asInstanceOf[Elem[_]] match {
    case _: ReverseIsoElem[A, B] @unchecked =>
      Some((p.asRep[ReverseIso[A, B]].iso))
    case _ =>
      None
  }
...
object Views_Module extends scalan.ModuleInfo {
  val dump = ""
}
}
