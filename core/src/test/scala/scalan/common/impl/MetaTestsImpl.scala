...
!!!("Cannot convert from MetaTest to MT1: missing fields List(data)")
  ...
  // elem for concrete class
  class MT2Elem[T, R](val iso: Iso[MT2Data[T, R], MT2[T, R]])(implicit elemT: Elem[T], elemR: Elem[R])
...
!!!("Cannot convert from MetaTest to MT2: missing fields List(indices, values)")
  ...
  // 3) Iso for concrete class
  class MT2Iso[T, R](implicit elemT: Elem[T], elemR: Elem[R])
    extends Iso[MT2Data[T, R], MT2[T, R]]()(pairElement(elemT, pairElement(elemR, element[Int]))) {
    ...
    lazy val tag = {
      implicit val tagT = elemT.tag
      implicit val tagR = elemR.tag
      weakTypeTag[MT2[T, R]]
    }
  ...
  abstract class MT2CompanionAbs extends CompanionBase[MT2CompanionAbs] with MT2Companion {
    ...
    def apply[T, R](p: Rep[MT2Data[T, R]])(implicit elemT: Elem[T], elemR: Elem[R]): Rep[MT2[T, R]] =
      isoMT2(elemT, elemR).to(p)
    def apply[T, R](indices: Rep[T], values: Rep[R], size: Rep[Int])(implicit elemT: Elem[T], elemR: Elem[R]): Rep[MT2[T, R]] =
  ...
  implicit class ExtendedMT2[T, R](p: Rep[MT2[T, R]])(implicit elemT: Elem[T], elemR: Elem[R]) {
    def toData: Rep[MT2Data[T, R]] = isoMT2(elemT, elemR).from(p)
  }

  // 5) implicit resolution of Iso
  implicit def isoMT2[T, R](implicit elemT: Elem[T], elemR: Elem[R]): Iso[MT2Data[T, R], MT2[T, R]] =
    new MT2Iso[T, R]

  // 6) smart constructor and deconstructor
  def mkMT2[T, R](indices: Rep[T], values: Rep[R], size: Rep[Int])(implicit elemT: Elem[T], elemR: Elem[R]): Rep[MT2[T, R]]
...
trait MetaTestsSeq extends MetaTestsDsl with ScalanSeq {
  ...
  case class SeqMT2[T, R]
      ...
      (implicit elemT: Elem[T], elemR: Elem[R])
  ...
  def mkMT2[T, R]
      (indices: Rep[T], values: Rep[R], size: Rep[Int])(implicit elemT: Elem[T], elemR: Elem[R]): Rep[MT2[T, R]] =
