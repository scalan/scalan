...
trait ItTestsUtil extends TestsUtil { self: Suite with Matchers =>
  ...
  def getStagedOutput[A, B](back: Compiler)(f: back.scalan.Exp[A => B], functionName: String, input: A): B =
    getStagedOutputConfig(back)(f, functionName, input, back.defaultCompilerConfig)

  def getStagedOutputConfig[A, B](back: Compiler)(f: back.scalan.Exp[A => B], functionName: String, input: A, compilerConfig: back.CompilerConfig): B = {
    val compiled = compileSource(back)(f, functionName, compilerConfig)
    back.execute(compiled, input)
  }

  def compileSource[A, B](back: Compiler)(f: back.scalan.Exp[A => B], functionName: String, compilerConfig: back.CompilerConfig) : back.CompilerOutput[A, B] = {
