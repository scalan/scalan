...
trait ConsoleCompiler extends ConsoleExp with Expressions with Compiler { self: ScalanExp =>

  object IoRewriter extends Rewriter {
  ...
  override def graphPasses(compilerConfig: CompilerConfig) =
    super.graphPasses(compilerConfig) :+
      constantPass(GraphTransformPass("io", DefaultMirror, IoRewriter))
}
