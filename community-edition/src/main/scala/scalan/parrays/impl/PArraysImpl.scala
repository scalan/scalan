...
trait PArraysAbs extends Scalan with PArrays {
  ...
  // elem for concrete class
  class PairArrayElem[A:Elem, B:Elem](iso: Iso[PairArrayData[A, B], PairArray[A, B]]) extends PArrayElem[(A,B), PairArrayData[A, B], PairArray[A, B]](iso) {
    def convertPArray(x: Rep[PArray[(A,B)]]) = // Converter is not generated by meta
!!!("Cannot convert from PArray to PairArray: missing fields List(as, bs)")
  }
  ...
  // elem for concrete class
  class NestedArrayElem[A:Elem](iso: Iso[NestedArrayData[A], NestedArray[A]]) extends PArrayElem[PArray[A], NestedArrayData[A], NestedArray[A]](iso) {
    def convertPArray(x: Rep[PArray[PArray[A]]]) = // Converter is not generated by meta
!!!("Cannot convert from PArray to NestedArray: missing fields List(values, segments)")
  }
