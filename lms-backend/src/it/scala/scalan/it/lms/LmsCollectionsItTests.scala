...
import scalan.collections.{SeqsScalaMethodMapping, CollectionsItTests, MultiMapItTests}
...
class LmsCollectionsItTests extends CollectionsItTests {
  ...
  val progStaged = new LmsCompilerScala(new ProgCommunityExp) with SeqsScalaMethodMapping
  val progStagedU = new LmsCompilerUni(new ProgCommunityExp) with SeqsScalaMethodMapping
