...
class UniCompilerItTests  extends LmsMsfItTests {
  ...
  test("ddmvm") {
    ...
    compareOutputWithSequential(progStaged)(progSeqU.ddmvm, progStaged.scalan.ddmvm, "ddmvm00", in)
  }
  ...
  ignore("msfFunAdjBase") {
    ...
    compareOutputWithSequential(progStaged)(progSeqU.msfFunAdjBase, progStaged.scalan.msfFunAdjBase, "msfFunAdjBase", input)
  }

  test("test00_nop") {
    val in = 5.0
    compareOutputWithSequential(progStaged)(progSeqU.test00_nop, progStaged.scalan.test00_nop, "test00_nop", in)
  }

  test("test01_oneOp") {
    val in = 5.0
    compareOutputWithSequential(progStaged)(progSeqU.test01_oneOp, progStaged.scalan.test01_oneOp, "test01_oneOp", in)
  }

  test("test02_mapArray") {
    val in = Array(2.0, 3.0)
    compareOutputWithSequential(progStaged)(progSeqU.test02_mapArray, progStaged.scalan.test02_mapArray, "test02_mapArray", in)
  }

  test("test03_zipArray") {
    val in = Array(2, 3)
    compareOutputWithSequential(progStaged)(progSeqU.test03_zipArray, progStaged.scalan.test03_zipArray, "test03_zipArray", in)
  }

  test("test04_zip2Arrays") {
    val in = (Array(2, 3), Array(1, 4))
    compareOutputWithSequential(progStaged)(progSeqU.test04_zip2Arrays, progStaged.scalan.test04_zip2Arrays, "test04_zip2Arrays", in)
  }

  test("test05_zip3Arrays") {
    val in = (Array(2, 3), (Array(1, 4), Array(0, 5)))
    compareOutputWithSequential(progStaged)(progSeqU.test05_zip3Arrays, progStaged.scalan.test05_zip3Arrays, "test05_zip3Arrays", in)
  }

  test("test06_simpleReduce") {
    val in = Array(2, 3)
    compareOutputWithSequential(progStaged)(progSeqU.test06_simpleReduce, progStaged.scalan.test06_simpleReduce, "test06_simpleReduce", in)
  }

  test("test07_reduceFromTuple") {
    compareOutputWithSequential(progStaged)(progSeqU.test07_reduceFromTuple, progStaged.scalan.test07_reduceFromTuple, "test07_reduceFromTuple", in3Arrays)
  }
  ...
  test("config_OnlyScala") {
    ...
    val file = compileSource(progStaged)(progStaged.scalan.test07_reduceFromTuple, "config_OnlyScala", config).custom.sources.head
    assert(linesWithNativeDef(file).size == 0)
    compareOutputWithSequentialConfig(progStaged)(progSeqU.test07_reduceFromTuple, progStaged.scalan.test07_reduceFromTuple, "config_OnlyScala", in, config)
  }
  ...
  test("config_Root") {
    ...
    val fileJ = compileSource(progStaged)(progStaged.scalan.test07_reduceFromTuple, "config_RootJ", config.copy(nativeMethods = noNative)).custom.sources.head
    assert(linesWithNativeDef(fileJ).size == 0)
    val fileC = compileSource(progStaged)(progStaged.scalan.test07_reduceFromTuple, "config_RootC", config.copy(nativeMethods = nativeRoot)).custom.sources.head
  ...
  test("config_MethodCall") {
    ...
    val config = progStaged.defaultCompilerConfig
    val fileJ = compileSource(progStaged)(progStaged.scalan.ddmvm, "config_MethodCallJ", config.copy(nativeMethods = noNative)).custom.sources.head
    assert(linesWithNativeDef(fileJ).size == 0)
    val fileC = compileSource(progStaged)(progStaged.scalan.ddmvm, "config_MethodCallC", config.copy(nativeMethods = nativeMul)).custom.sources.head
    assert(linesWithNativeDef(fileC).size == 1)
    // todo check, that main method is not native
    assert(linesWithNativeDef(fileC).size-100 == 1)

    compareOutputWithSequentialConfig(progStaged)(progSeqU.ddmvm, progStaged.scalan.ddmvm, "config_MethodCall", in, config.copy(nativeMethods = nativeMul))
  ...
  test("config_MarkedAsNativeLambda") {
    ...
    compareOutputWithSequentialConfig(progStaged)(progSeqU.test_config, progStaged.scalan.test_config, "config_MarkedAsNativeLambda", in3Arrays, config.copy(nativeMethods = native))
  }
