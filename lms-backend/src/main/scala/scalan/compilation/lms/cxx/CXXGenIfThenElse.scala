 ...
 */
trait CXXGenIfThenElse  extends CLikeGenEffect with BaseGenIfThenElse {
  import IR._
...
trait CXXGenIfThenElseFat extends CXXGenIfThenElse with CLikeGenFat with BaseGenIfThenElseFat {
  ...
  override def emitFatNode(symList: List[Sym[Any]], rhs: FatDef) = rhs match {
    case SimpleFatIfThenElse(c,as,bs) =>
      ...
      emitFatBlock(as)
      (symList zip as.map(getBlockResult)).foreach( {p => emitAssignment(p._1, s"std::move(${quote(p._2)})")} )
      stream.println("} else {")
      emitFatBlock(bs)
      (symList zip bs.map(getBlockResult)).foreach( {p => emitAssignment(p._1, s"std::move(${quote(p._2)})")} )
      stream.println("}")
