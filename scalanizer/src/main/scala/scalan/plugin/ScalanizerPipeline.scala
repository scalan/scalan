...
class ScalanizerPipeline[+G <: Global](s: Scalanizer[G]) extends ScalanPluginPipeline[G](s) {
  ...
  val steps: List[Phase] = List(
    ForEachUnit("wrapfrontend") { context => import context._;
    ...
    ForEachUnit("virtfrontend") { context => import context._;
      val unitName = unit.source.file.name
      if (isModuleUnit(unitName)) {
    ...
    ForEachUnit("final") { context => import context._
